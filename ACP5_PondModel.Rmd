...
Followingly, the ensemble was run with the calibrated parameters.

```{r bestparamrun}
config_file = "LakeEnsemblR_EERES_calibrated.yaml"
model = c("FLake","GOTM","Simstrat")
export_config(config_file = config_file, model = model)
run_ensemble(config_file = config_file, model = model, parallel=T, ncore=3)
```
...
We see that only the model FLake is able to provide results with a vertical stratification of water temperature. All models seem to show lower temperatures. To further investigate this, we looked at the water temperature time series at each depth against the observed time series.
...
plot30 = plot_ensemble(ncdf,model,var="temp",depth=0.30)

plot5 + plot10 + plot15 + plot20 + plot25 + plot30 + plot_layout(ncol=2)
```

For all depths, the models underestimate the water temperature. Especially during the winter months and in the deeper layers, the models show a stronger underestimation of the temperature. GOTM has lower variability in the winter months due to a very low temperature that is limited at 0° C.

This can be inspected in detail by
